...
namespace TaxiApp.Backend.Infrastructure.Helper
{
    ...
    .ThenBy(o => o.TripOfferSentAt) // الأقدم أول
                ...
                // =========================
                // 1.5️⃣ Activate scheduled orders whose time has arrived
                // =========================
                var dueScheduledOrders = await db.Orders
                ...
                foreach (var order in dueScheduledOrders)
                {
                    try
                    {
                        await driverAssignmentService.DriverAssignAsync(order);
                        await Task.Delay(500, stoppingToken);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, "خطأ أثناء تفعيل الطلب المجدول رقم {OrderId}", order.OrderId);
                    }
                }
