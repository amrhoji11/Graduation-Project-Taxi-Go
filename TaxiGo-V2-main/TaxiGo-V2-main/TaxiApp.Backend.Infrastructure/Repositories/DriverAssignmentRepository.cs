        ...
        public async Task<string> AssignTripEmergencyAsync(int tripId, string? excludedDriverId = null)
        {
            ...
            var drivers = await _context.Drivers
                ...
                .Where(d =>
                    ...
                    d.Status != DriverStatus.returningToOffice)
                ...
                if (queueDriver == null)
                {
                    ...
                    foreach (var tripOrder in trip.TripOrders)
                    {
                        await _notification.SendNotificationAsync(
                            ...
                            NotificationType.NoDriverFound,
                            "نعتذر منك",
                            "تعذر العثور على سائق لإكمال رحلتك",
                            tripOrder.OrderId,
                            trip.TripId
                        );
                    }
        ...
        private async Task<string> AssignOrderUrgentAsync(Order order, string? excludedDriverId = null)
        {
            ...
            var drivers = await _context.Drivers.AsNoTracking()
                .Where(d =>
                    ...
                    d.Status != DriverStatus.offline &&
                    d.Status != DriverStatus.returningToOffice &&
                    _context.DriverApprovals.Any(a => a.DriverId == d.UserId && a.Status == ApprovalStatus.approved) &&
                    !_context.UserBlocks.Any(b => b.UserId == d.UserId && b.StartsAt <= now && (b.EndsAt == null || b.EndsAt > now)))
                .Select(d => new
                {
                    Driver = d,
                    Vehicle = d.Vehicles
...
.ToDictionary(g => g.Key, g => g.First());
    ...
    new
    {
        ...
        private async Task<List<DriverCandidate>> GetNearestDrivers(Order order, TimeSpan maxEta, string? excludedDriverId)
        {
            ...
            // 1️⃣ جلب السائقين
            // =========================
            // Eligibility: approved by an Admin and not currently blocked,
            // on top of the existing available/active/seen-recently checks
            // - `Status == available` alone never guaranteed either of
            // these (see `EnterQueueAsync`), so an unapproved or blocked
            // driver could otherwise still be offered a trip.
            var drivers = await _context.Drivers
                .AsNoTracking()
                .Where(d =>
        ...
        public async Task<string> DriverAcceptOrderAsync(int  orderId, string driverId)
        {
          ...
          .FirstOrDefaultAsync(o => o.OrderId == orderId);
            ...
            // =========================
            // 🚀 Driver must be free - a driver already mid-trip should
            // never have been offered another order (GetNearestDrivers only
            // offers to DriverStatus.available), but this guards against it
            // defensively instead of bundling the new order into the
            // existing trip (the old shared-trip pooling behavior, removed).
            // =========================
            if (activeTrip != null)
            {
            ...
            // =========================
            // 🚀 Create a new single-order trip
            // =========================
            {
       ...
       .FirstOrDefaultAsync(o => o.OrderId == orderId);
        ...
        public async Task<string> EnterQueueAsync(string driverId)
        {
            ...
            // `available` is the resting "ready for orders" state, but a
            // freshly-approved driver defaults to `offline` and nothing else
            // ever moved them to `available` for the first time - this used
            // to require `Status == available` just to enter the queue,
            // making the driver's Active/Inactive toggle permanently
            // unreachable for any driver who hadn't already completed a trip.
            if (driver == null ||
                driver.Status == DriverStatus.busy ||
            ...
            var isBlocked = await _context.UserBlocks.AnyAsync(b =>
                b.UserId == driverId && b.StartsAt <= now && (b.EndsAt == null || b.EndsAt > now));
       ...
       saveToDb: true
        ...
        /// The inverse of [EnterQueueAsync] - the driver's "go inactive"
        /// action. Blocked while mid-trip (`busy`/`Shared`); harmless to call
        /// when already `offline` so the client can use it defensively.
        public async Task<string> LeaveQueueAsync(string driverId)
        {
            var driver = await _context.Drivers.FindAsync(driverId);
            ...
            if (driver.Status == DriverStatus.busy || driver.Status == DriverStatus.Shared)
                return "can not leave queue while on an active trip";

            var entry = await _context.OfficeQueueEntries
                .FirstOrDefaultAsync(q => q.DriverId == driverId && q.Status == QueueStatus.InQueue);
        ...
        /// Driver-initiated action for after a dropoff: "I'm done with this
        /// trip, heading back to the office" - distinct from `offline` so
        /// the app/admin can show it correctly, but excluded from automatic
        /// matching exactly like `offline` is. Only valid from `available`
        /// (i.e. not mid-trip) so it can't interfere with an in-progress
        /// trip's own status transitions. The driver comes back via the
        /// existing [EnterQueueAsync] ("Go active") action, which already
        /// accepts any status except busy/Shared/rejected.
        public async Task<string> ReturnToOfficeAsync(string driverId)
        {
            var driver = await _context.Drivers.FindAsync(driverId);
            ...
            if (driver.Status != DriverStatus.available)
                return "can only mark returning to office when available";
            ...
            driver.Status = DriverStatus.returningToOffice;

            await _context.SaveChangesAsync();

            return "Marked as returning to office";
        }

        /// Lightweight "something about this trip changed, refresh" signal
        /// for the admin live-trips screen - deliberately doesn't carry the
        /// new status itself (that would duplicate the `DetailedStatus`
        /// derivation logic in `AdminRepository.GetCurrentTripsAsync` here
        /// too, and risk the two drifting out of sync). The client re-fetches
        /// that trip's real state from the REST endpoint on receipt, same as
        /// the existing polling already does, just triggered immediately
        /// instead of waiting for the next tick. Never throws - a failed
        /// broadcast must not fail the actual trip action that triggered it.
        private async Task NotifyOfficeTripChangedAsync(int tripId)
        {
            try
            {
                await _hubContext.Clients.Group(HubGroups.Office)
                    .SendAsync("AdminTripStatusChanged", new { tripId });
            }
            catch
            {
                // Best-effort only - polling remains the reliable fallback.
            }
        }
        ...
        private async Task<Driver?> GetNextDriverFromQueueAsync()
        {
            ...
            var entry = await _context.OfficeQueueEntries
                .Where(q =>
                    q.Status == QueueStatus.InQueue &&
                    q.Driver.Status == DriverStatus.available &&
                    _context.DriverApprovals.Any(a => a.DriverId == q.DriverId && a.Status == ApprovalStatus.approved) &&
                    !_context.UserBlocks.Any(b => b.UserId == q.DriverId && b.StartsAt <= now && (b.EndsAt == null || b.EndsAt > now)))
    ...
    .FirstOrDefaultAsync(to =>
     ...
     extraData: new
     {
         ...
         destination = tripOrder.Order.DropoffLocation
            ...
            await NotifyOfficeTripChangedAsync(trip.TripId);
        ...
        public async Task<string> StartTripAsync(int tripId, string driverId)
        {
          ...
          .FirstOrDefaultAsync(t =>
            ...
            await NotifyOfficeTripChangedAsync(tripId);
   ...
   .FirstOrDefaultAsync(to =>
    ...
    tripOrder.StatusInTrip != TripOrderStatus.DriverArrived)
            ...
            // Recalculate unconditionally - BuildTripRoute now plans the
            // dropoff leg too, and that leg only exists once the order is
            // actually picked up.
     ...
     extraData: new
     {
         ...
         countdown = 1800 // 30 دقيقة
            ...
            {
                // Defensive only - every new trip has exactly one order, so
                // this branch (another order in the same trip still active)
                // is unreachable going forward; kept for any legacy
                // multi-order trip rows.
                driver.Status = DriverStatus.busy;
