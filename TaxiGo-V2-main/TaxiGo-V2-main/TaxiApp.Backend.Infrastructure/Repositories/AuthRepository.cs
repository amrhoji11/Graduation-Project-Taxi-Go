...
namespace TaxiApp.Backend.Infrastructure.Repositories
{
    public class AuthRepository : IAuthRepository
    {
        ...
        public async Task<LoginResponse> VerifyOtpAndLoginAsync(VerifyOtpRequest request)
        {
            ...
            if (user.IsDeleted)
                throw new Exception("هذا الحساب محذوف.");

            if (!user.IsActive)
                throw new Exception("حسابك غير نشط، يرجى التواصل مع الدعم.");

            var isBlocked = await _context.UserBlocks
                .AnyAsync(b => b.UserId == user.Id && (b.EndsAt == null || b.EndsAt > DateTime.UtcNow));

            if (isBlocked)
                throw new Exception("حسابك محظور، لا يمكنك تسجيل الدخول.");
