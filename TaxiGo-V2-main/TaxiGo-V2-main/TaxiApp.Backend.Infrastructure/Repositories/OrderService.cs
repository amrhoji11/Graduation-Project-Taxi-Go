...
namespace TaxiApp.Backend.Infrastructure.Repositories
{
    ...
    IMapService mapService,INotificationRepository notificationRepository, IOptions<TaxiSettings> settings)
        ...
        public async Task<Order> CreateAndAssign(string userId, CreateOrderDto dto)
        {
            ...
            // طلب مجدول لوقت لاحق - لا تبحث عن سائق الآن، الـ
            // TripOfferBackgroundService هو من يفعّل البحث عند حلول الوقت
            var isScheduledForLater = order.ScheduledAt.HasValue && order.ScheduledAt.Value > DateTime.UtcNow;

            if (_settings.EnableAutoAssignment && !isScheduledForLater)
            {
