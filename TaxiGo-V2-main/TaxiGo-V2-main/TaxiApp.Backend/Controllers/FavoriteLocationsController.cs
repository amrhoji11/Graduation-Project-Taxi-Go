...
namespace TaxiApp.Backend.Api.Controllers
{
    ...
    public class FavoriteLocationsController : ControllerBase
    {
        ...
        // تعديل موقع مفضل
        [HttpPut("UpdateFavoriteLocation/{locationId}")]
        public async Task<IActionResult> UpdateLocation(int locationId, [FromBody] UpdateFavoriteLocationDto dto)
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            ...
            var success = await favoriteLocationsRepository.UpdateLocationAsync(userId, locationId, dto);

            if (!success)
                return NotFound(new { message = "الموقع غير موجود" });

            return Ok(new { message = "تم تعديل الموقع المفضل" });
        }
