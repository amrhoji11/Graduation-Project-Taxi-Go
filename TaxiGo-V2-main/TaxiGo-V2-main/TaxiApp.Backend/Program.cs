...
namespace TaxiApp.Backend
{
    public class Program
    {
        public static async Task Main(string[] args)
        {
           ...
           // Short timeout: this client is awaited synchronously inside
           // CreateOrder/DriverAssignAsync before the HTTP response is sent.
           // The default HttpClient.Timeout (100s) would let a slow/unreachable
           // public OSRM demo server hang the whole order-creation request far
           // past the mobile client's own request timeout, surfacing as a
           // frozen Create Order screen. Failing fast keeps the existing
           // try/catch degrade-to-empty/MaxValue fallback (GetNearestDrivers
           // already skips drivers with TimeSpan.MaxValue ETA) responsive.
           builder.Services.AddHttpClient<IMapService, OsrmMapService>(client =>
           {
